...
pub fn run_opt_pruned(bids: Stream<Bid>, size: usize, step: usize, ctx: &mut Context) {
    let bids = bids.map(ctx, |a| {
        PrunedBid::new(a.price)
    });
    bids.count_sliding_holistic_window(ctx, size, step, |data| {
        let mut sum = 0;
        let mut count = 0;
...
pub fn run_opt_operator(bids: Stream<Bid>, size: usize, step: usize, ctx: &mut Context) {
